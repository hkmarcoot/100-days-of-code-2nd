...
describe("", function () {
  it("", async function () {
    ...
    expect(
      orderSunglasses,
      "`orderSunglasses()` should be a function."
    ...
    try {
      ...
      expect(
        true,
        "Did you accidentally delete your `myExecutor()` function?"
      ).to.equal(false);
    }
    let inventory;
    try {
      ...
      expect(
        true,
        "Did you accidentally delete the `inventory` object?"
      ).to.equal(false);
    }
    ...
    expect(
      testPromise === "Sunglasses order processed.",
      "`orderSunglasses()` should return a promise constructed with `new Promise(myExecutor)`."
    ).to.equal(true);
  });
});
