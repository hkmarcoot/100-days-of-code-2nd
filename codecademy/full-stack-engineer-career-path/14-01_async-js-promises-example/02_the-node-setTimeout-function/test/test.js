...
describe("", function () {
  it("", function () {
    ...
    let varCallbacks = [
      function ($func, $num) {
        if ($func.name !== "usingSTO") {
          return {
            failure:
              "Did you pass `usingSTO()` uninvoked as the first argument in your `setTimeout()` call?",
          };
        ...
        if (!valTest) {
          return {
            failure:
              "Did you pass a number between `0` and `3000` as the second argument in your `setTimeout()` call?",
          };
    ...
    assert.isOk(
      isMatchOne,
      varCallbacks.failure || "Did you call `setTimeout()`?"
    );
  });
});
